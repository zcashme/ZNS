...
async fn submit_funding(
    ...
    let payout_tx = payout::build_tx_bytes(PayoutInputs {
        ...
        buyer_ua: None,
        seller_amount: contract_listing.seller_receives_zat(state.commission_bps, crate::payout::payout_fee()),
        treasury_amount: contract_listing.commission_zat(state.commission_bps),
        memo: build_buy_memo(
            &listing.name,
            &reg.buyer_ua,
