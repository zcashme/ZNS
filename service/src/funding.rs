//! Background worker that drives every active buyer registration through to
//! a settled payout.
//!
//! For each registration in the local DB:
//!
//!   * **AwaitingPayment** — derive the burner t-address locally, poll it;
//!     when an exact-amount UTXO with enough confirmations lands, build the
//!     payout tx, call `submit_funding` on chain, then
//!     `request_payout_signature`.
//!   * **Submitted**       — poll `get_payout_signature`; once available,
//!     finalize the tx and broadcast via lightwalletd.
//!   * **Completed**       — terminal.
//!
//! Refunds for wrong-amount or losing-race UTXOs are out of scope here and
//! sit at the burner address until a future flow handles them.

use std::{sync::Arc, time::Duration};

use anyhow::{Context, Result, bail};
use near_mpc_crypto_types::K256Signature;
use near_primitives::views::FinalExecutionStatus;
use serde_json::json;
use zcash_transparent as transparent;

use crate::{
    burner::derive_burner,
    contract_view::ContractListingMeta,
    db::{BuyerRegistration, RegistrationStatus},
    http::AppState,
    payout::{self, PayoutInputs},
    zcash::EscrowUtxo,
};

const SUBMIT_FUNDING_GAS: u64 = 150_000_000_000_000;
const SIGN_REQUEST_GAS: u64 = 300_000_000_000_000;

pub async fn run_worker(state: Arc<AppState>) {
    let interval = Duration::from_secs(state.cfg.poll_interval_secs.max(1));
    loop {
        match state.store.list_active_registrations() {
            Ok(registrations) => {
                for reg in registrations {
                    if let Err(e) = process_registration(&state, reg).await {
                        tracing::error!(error = %e, "funding worker failed for registration");
                    }
                }
            }
            Err(e) => tracing::error!(error = %e, "load active registrations failed"),
        }
        tokio::time::sleep(interval).await;
    }
}

struct DerivedBurner {
    taddr: String,
    pubkey: [u8; 33],
}

fn derive_for(state: &AppState, reg: &BuyerRegistration) -> Result<DerivedBurner> {
    let (taddr, pubkey) = derive_burner(
        &state.mpc_root_pubkey,
        &state.contract_account,
        &reg.name,
        reg.listing_nonce,
        state.mainnet,
    )?;
    Ok(DerivedBurner { taddr, pubkey })
}

async fn process_registration(state: &Arc<AppState>, reg: BuyerRegistration) -> Result<()> {
    let burner = derive_for(state, &reg)?;
    let contract_meta = fetch_contract_meta(state, reg.contract_listing_id)
        .await?
        .context("contract listing not found")?;

    match reg.status {
        RegistrationStatus::AwaitingPayment => {
            handle_awaiting_payment(state, &reg, &burner, &contract_meta).await
        }
        RegistrationStatus::Submitted => drive_payout(state, &reg, &burner).await,
        RegistrationStatus::Completed => Ok(()),
    }
}

async fn handle_awaiting_payment(
    state: &Arc<AppState>,
    reg: &BuyerRegistration,
    burner: &DerivedBurner,
    contract_meta: &ContractListingMeta,
) -> Result<()> {
    if contract_meta.funded {
        // Listing was funded by someone else — our registration is moot.
        return Ok(());
    }

    let Some(utxo) = find_exact_funding_utxo(state, &burner.taddr, reg.price_zat).await? else {
        return Ok(());
    };

    submit_funding(state, reg, burner, &utxo).await?;
    request_payout_signature(state, reg.contract_listing_id).await?;
    Ok(())
}

async fn submit_funding(
    state: &Arc<AppState>,
    reg: &BuyerRegistration,
    burner: &DerivedBurner,
    utxo: &EscrowUtxo,
) -> Result<()> {
    let build_height = state.watcher.latest_height().await? as u32;
    let outpoint = transparent::bundle::OutPoint::new(txid_le(&utxo.txid)?, utxo.vout);

    let fee = crate::payout::payout_fee();
    let commission = reg.price_zat * state.commission_bps / 10_000;
    let seller_amount = reg.price_zat.saturating_sub(commission).saturating_sub(fee);

    let payout_tx = payout::build_tx_bytes(PayoutInputs {
        network: &state.cfg.zcash_network,
        target_height: build_height,
        utxo_outpoint: outpoint,
        utxo_value_zats: utxo.value_zats,
        utxo_script_pubkey: utxo.script.clone(),
        burner_pubkey: burner.pubkey,
        seller_ua: &reg.seller_ua,
        treasury_ua: &state.treasury_ua,
        buyer_ua: None,
        seller_amount,
        treasury_amount: commission,
        memo: build_buy_memo(
            &reg.name,
            &reg.buyer_ua,
            &reg.admin_signature_b64,
            reg.is_sovereign.then_some(reg.buyer_pubkey_b64.as_str()),
        ),
        fee: Some(fee),
    })?;

    // Sovereign signatures are flagged in the DB so the contract can verify
    // them on chain. The admin signature is always in the memo for anti-forgery.
    let sovereign_pk = if reg.is_sovereign {
        Some(reg.buyer_pubkey_b64.as_str())
    } else {
        None
    };

    call_contract_method(
        state,
        "submit_funding",
        json!({
            "listing_id": reg.contract_listing_id,
            "utxo_value_zats": utxo.value_zats,
            "utxo_script_pubkey": utxo.script,
            "payout_tx": payout_tx,
            "buyer_ua": reg.buyer_ua,
            "admin_signature_b64": reg.admin_signature_b64,
            "buyer_pubkey_b64": sovereign_pk,
        }),
        SUBMIT_FUNDING_GAS,
        0,
    )
    .await?;

    state
        .store
        .mark_registration_submitted(reg.id, &utxo.txid, utxo.vout, build_height)?;

    Ok(())
}

async fn drive_payout(
    state: &Arc<AppState>,
    reg: &BuyerRegistration,
    burner: &DerivedBurner,
) -> Result<()> {
    if reg.payout_txid.is_some() {
        return Ok(());
    }

    let sig = fetch_payout_signature(state, reg.contract_listing_id).await?;
    let Some(sig) = sig else {
        // Signature not yet available — re-request to nudge the MPC.
        request_payout_signature(state, reg.contract_listing_id).await?;
        return Ok(());
    };

    let utxo = find_stored_utxo(state, reg, &burner.taddr)
        .await?
        .context("stored funding utxo not found")?;
    let build_height = reg
        .build_height
        .context("registration missing build_height")? as u32;
    let outpoint = transparent::bundle::OutPoint::new(txid_le(&utxo.txid)?, utxo.vout);

    let fee = crate::payout::payout_fee();
    let commission = reg.price_zat * state.commission_bps / 10_000;
    let seller_amount = reg.price_zat.saturating_sub(commission).saturating_sub(fee);

    let plan = payout::build_unsigned(PayoutInputs {
        network: &state.cfg.zcash_network,
        target_height: build_height,
        utxo_outpoint: outpoint,
        utxo_value_zats: utxo.value_zats,
        utxo_script_pubkey: utxo.script,
        burner_pubkey: burner.pubkey,
        seller_ua: &reg.seller_ua,
        treasury_ua: &state.treasury_ua,
        buyer_ua: None,
        seller_amount,
        treasury_amount: commission,
        memo: build_buy_memo(
            &reg.name,
            &reg.buyer_ua,
            &reg.admin_signature_b64,
            reg.is_sovereign.then_some(reg.buyer_pubkey_b64.as_str()),
        ),
        fee: Some(fee),
    })?;

    let final_tx = payout::finalize_with_mpc(plan, &signature_to_compact(&sig)?)?;
    let txid = state.watcher.send_tx(&final_tx).await?;
    state.store.mark_registration_completed(reg.id, &txid)?;
    Ok(())
}

async fn find_exact_funding_utxo(
    state: &Arc<AppState>,
    burner_taddr: &str,
    price_zat: u64,
) -> Result<Option<EscrowUtxo>> {
    let mut utxos = state.watcher.get_utxos(burner_taddr).await?;
    utxos.retain(|u| u.value_zats == price_zat && u.confirmations >= state.cfg.min_confirmations);
    utxos.sort_by_key(|u| std::cmp::Reverse(u.confirmations));
    Ok(utxos.into_iter().next())
}

async fn find_stored_utxo(
    state: &Arc<AppState>,
    reg: &BuyerRegistration,
    burner_taddr: &str,
) -> Result<Option<EscrowUtxo>> {
    let Some(funding_txid) = reg.funding_txid.as_ref() else {
        return Ok(None);
    };
    let Some(funding_vout) = reg.funding_vout else {
        return Ok(None);
    };

    let utxos = state.watcher.get_utxos(burner_taddr).await?;
    Ok(utxos.into_iter().find(|u| {
        u.txid == *funding_txid && u.vout == funding_vout as u32 && u.value_zats == reg.price_zat
    }))
}

async fn fetch_contract_meta(
    state: &Arc<AppState>,
    contract_listing_id: i64,
) -> Result<Option<ContractListingMeta>> {
    state
        .near
        .view_zns("get_listing", json!({ "id": contract_listing_id }))
        .await
        .context("view get_listing")
}

async fn fetch_payout_signature(
    state: &Arc<AppState>,
    contract_listing_id: i64,
) -> Result<Option<K256Signature>> {
    state
        .near
        .view_zns(
            "get_payout_signature",
            json!({ "listing_id": contract_listing_id }),
        )
        .await
        .context("view payout signature")
}

async fn request_payout_signature(state: &Arc<AppState>, contract_listing_id: i64) -> Result<()> {
    call_contract_method(
        state,
        "request_payout_signature",
        json!({ "listing_id": contract_listing_id }),
        SIGN_REQUEST_GAS,
        0,
    )
    .await
}

async fn call_contract_method(
    state: &Arc<AppState>,
    method: &str,
    args: serde_json::Value,
    gas: u64,
    deposit: u128,
) -> Result<()> {
    let outcome = state.near.call_zns_mut(method, args, gas, deposit).await?;
    match outcome.status {
        FinalExecutionStatus::SuccessValue(_) => Ok(()),
        FinalExecutionStatus::Failure(err) => bail!("{method} failed: {err:?}"),
        other => bail!("{method} unexpected status: {other:?}"),
    }
}

/// Build the BUY memo placed on the treasury Orchard output.
///
/// Admin format:    `ZNS:BUY:<name>:<buyer_ua>:<admin_sig_b64>`
/// Sovereign format: `ZNS:BUY:<name>:<buyer_ua>:<admin_sig_b64>:<buyer_pubkey_b64>`
fn build_buy_memo(
    listing_name: &str,
    buyer_ua: &str,
    admin_signature_b64: &str,
    buyer_pubkey_b64: Option<&str>,
) -> [u8; 512] {
    let memo = match buyer_pubkey_b64 {
        Some(pk) => format!("ZNS:BUY:{listing_name}:{buyer_ua}:{admin_signature_b64}:{pk}"),
        None => format!("ZNS:BUY:{listing_name}:{buyer_ua}:{admin_signature_b64}"),
    };
    let mut bytes = [0u8; 512];
    let raw = memo.as_bytes();
    let len = raw.len().min(512);
    bytes[..len].copy_from_slice(&raw[..len]);
    bytes
}

fn txid_le(txid_be_hex: &str) -> Result<[u8; 32]> {
    let mut bytes = hex::decode(txid_be_hex).context("decode funding txid")?;
    if bytes.len() != 32 {
        bail!("funding txid must be 32 bytes");
    }
    bytes.reverse();
    let mut out = [0u8; 32];
    out.copy_from_slice(&bytes);
    Ok(out)
}

fn signature_to_compact(sig: &K256Signature) -> Result<[u8; 64]> {
    let mut compact = [0u8; 64];
    compact[..32].copy_from_slice(&sig.big_r.affine_point[1..]);
    compact[32..].copy_from_slice(&sig.s.scalar);
    Ok(compact)
}
